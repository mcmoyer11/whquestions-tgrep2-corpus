...
length(unique(failed_controls$workerid)) # 37
length(unique(failed_controls$workerid))/length(unique(d$workerid))*100 # 3.5%
...
test$QuestionType[test$EmbeddedSQ == "yes"] <- "embeddedSAI"
test$EmbeddedSQ = as.factor(test$EmbeddedSQ)
test$QuestionType = as.factor(test$QuestionType)

table(test$QuestionType)
length(unique(test$workerid))
length(unique(test$workerid[test$QuestionType!="embeddedSAI"])) 
```
...
# ggsave("../graphs/1b_overall_raw.pdf",width=3.5,height=3)
```
...
nrow(other_ratings)/nrow(test_agr)*100 # 15.5%
nrow(test_agr) # 4052

or_ids = other_ratings$tgrep_id
test_other = test %>%
  filter(tgrep_id %in% or_ids)
nrow(test_other)/nrow(test)*100 # 15.5
...
nrow(test_norm) # 95956

# write.csv(test_norm,"../data/embedded_no_rhetorical.csv")
```

# Looing at QuestionType
## Look at SAI and Particle Verbs in Rhetorical Qs
```{r, include = FALSE}
...
## Look at SAI and Particle Verbs in Test Qs
```{r, include = FALSE}
...
## Look at questions in test data without embedded SAI
...
## Graph Embedded Subject-Aux Inversion
```{r, include = FALSE}
agr = test_norm %>%
...
# Focusing just on "Every", "a" and "the"
## Normalize ratings to make a probability distribution
...
surp = subset(normed,normed$VerbLemma == "surprise")
nrow(surp)
agr = normed %>%
  # Most frequent
