...
\mm{better to show most frequen or the theoretically interesting verbs?}
\begin{figure}[h!]
\centering
\includegraphics[scale=.6]{figures/matrixverbs_frequent_context.pdf}
\caption{Mean ratings by paraphrase for the most frequent Matrix Verbs in the corpus, in Experiment 1b. Error bars indicate 95\% bootstrapped confidence intervals.}
...
\mm{FINISH WITH THE MODEL RESULTS HERE}
When we run our submodels for \emph{know-wh} questions, we find the following.
for 
\emph{what}
\emph{how}
\emph{where}
...
The most robust finding is that the presence of a modal significantly increases ratings for MS than the absence. We coded simply for presence versus absense of modality as a factor without differentiating between modal flavor or force. We found influence of modality even without a more fine-graned distinction. However, the literature focuses on the auxiliary \emph{can} as an existential possibility modal (\cite{george2011,fox2014,nicolae2014,xiang2016}\mm{more recent papers fox2018,xiang 2020}), and other modals are not discussed. By attributing the modal effect to existential possibility permits us to reduce the mention-some phenomenon to sometime existential (for, though we didn't discuss in detail, recall from section \mm{XX} that questions with existential quantification give rise to MS). This alignment also predicts that existential modals will give rise to MS. Is this indeed the case? \citeA{dayal2016} (section 2.3) concludes that in fact rather than existential force, a priority modal flavor (a term from \citeA{portner2009}) is what drives MS. Priority flavor refers to necessary or possible ways of achieving a goal or priority that is set by context. They have a circumstantial modal base and an ordering source, both of which are determined by the context.
% Discussion of dayal's examples?
% Dayal poses two questions 

What we would predict in our data is the following. On the hypothesis that MS is linked to existential or possibility modality, then we should find higher ratings for \emph{a} than \emph{every} for \emph{might}, \emph{may} in addition to \emph{can} and \emph{could}; while universal or necessity modals like \emph{should}, \emph{shall}, \emph{must}, \emph{ought} should show the reverse pattern, \emph{every} ratings should be higher than \emph{a} ratings. In contrast, if the priority flavor of the modal is relevant, then quantificational force won't be the deciding factor. Thus, if the contextual goals support a mention-some priority or goal, we should get a mention-some reading even in universal or necessissity modal questions.

Turning to the current data, figure \ref{exs1_modals} breaks down our results by modal auxiliary (note, we include here the category 'nonfinite' because non-finite embedded clauses have long been analyzed as containing covert modality \cite{bhatt1999,portner2009}\mm{others?}.

\mm{finish discussion here}
\begin{figure}[h!]
\centering
\includegraphics[scale=1]{figures/modals_context.pdf}
\caption{Mean ratings by modality across Experiments 1a and 1b.} 
\label{exs1_modals}
\end{figure}

Our results seem to show that mention-some acceptability cuts across both force and flavor. First, \emph{a} paraphrases are more acceptable than \emph{every} paraphrases for the epistemic possibility modals \emph{may} and \emph{might}
...
\caption{Mean ratings by modality across Experiments 2a and 2b.} 
\label{exs2_modals}
\end{figure}
...
Table \ref{sub-model_Task} presents regression results assessing the effect of Context (Task) on \emph{a}- and \emph{every}- paraphrase ratings. We find first that there is a significant initial bias for \emph{a}- over \emph{every}-paraphrases, indicated by the positive coefficient for Paraphrase, as well as a significant initial bias in the Context over the No Context Task \mm{wait, is this right? because Paraphrase is centered, th}
