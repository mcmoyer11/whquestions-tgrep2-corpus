...
ggsave("../graphs/1a_overall_raw.pdf",width=3.5,height=3)
```
...
ggplot(agr,aes(x=paraphrase, y=mean_rating, fill=ModalPresent)) +
  ...
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette)
# ggsave("../graphs/test_ModxWh-raw.pdf")
...
ggplot(agr, aes(x=paraphrase, y=mean_rating,fill=paraphrase, color=paraphrase)) + 
    ...
    stat_summary(fun.data = data_summary, geom = "pointrange",
                # width = 0.2) +
  # stat_sum_df("mean_cl_boot", mapping = aes(paraphrase))
  ylim(0,1) +
  scale_fill_manual(values=cbPalette) +
  scale_color_manual(values=cbPalette)
...
sum <- agr %>% 
  group_by(paraphrase) %>% 
  summarize(GrandMean = mean(mean_rating), CILow = ci.low(mean_rating), CIHigh = ci.high(mean_rating)) %>%
  mutate(YMin = GrandMean - CILow, YMax = GrandMean + CIHigh) %>%
  drop_na()
...
ggplot(agr, aes(x=paraphrase, y=mean_rating,fill=paraphrase, color=paraphrase)) + 
  ...
  # stat_summary(fun.data = sum, geom="point", size=3) +
