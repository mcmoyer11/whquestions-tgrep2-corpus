---
title: "Experiment 2b: Analysis of Embedded Questions, NoContext"
author: "mcmoyer"
date: "March 30, 2021"
...
# Is rating modulated by Paraphrase, Wh and ModalPresent?
...
- No, but bias for MS
```{r}
# m.ms.ma = lmer(normed_rating ~ cParaphrase + (1+cParaphrase|workerid) + (1+cParaphrase|VerbLemma) + (1+cParaphrase|tgrep_id), data=d.ms.ma)
...
- but significant effects both of Wh-Word and ModalPresent
  - Modal increases ratings
  - compared to 'when' all others lead to a decrease in ratings
```{r}
d.ms = d %>% 
  filter(paraphrase=="a") %>% 
  mutate(cModalPresent = as.numeric(ModalPresent)-mean(as.numeric(ModalPresent))) %>%
...
# m.ma = lmer(normed_rating ~ cModalPresent*Wh + (1+Wh+cModalPresent|workerid) + (1|tgrep_id) + (1|VerbLemma), data=d.ma)
...
- some 2-way interactions: only between paraphrase and Wh (how vs when, a vs every; why vs when, the versus every
```{r, eval=FALSE, warning=FALSE, echo=FALSE}
...
# saveRDS(m.full, "model-nocontext-embedded-full-rMV.rds")

nocontext_model <- readRDS("model-nocontext-embedded-full-rMV.rds")
